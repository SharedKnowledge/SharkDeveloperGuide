...
\item[times] A time semantic tag set containing time periods which are of any interest for the peer.
\end{description}
...
Lets review our last example. We have created a context point that has a group in its peer dimension. That's usual and advisable. It is convenient and efficient to create groups and add peers. Afterwards, we can add information and use those groups to define what peers (plural!) shall get these information.
...
Let's see what happens afterwards in this example:
...
An empty knowledge base ({\tt kb1}) is created. Afterwards, the assimilation is started and uses the empty knowledge base as target and the knowledge as source. 
